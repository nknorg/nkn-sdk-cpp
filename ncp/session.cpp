...
Session::Session(const string& localAddr, const string& remoteAddr,
    ...
    : config(Config::MergeDefaultConfig(cfg))
        ...
        , recvMtu(config->MTU)
        , resendChan(nullptr)
        , isAccepted(false) , isEstablished(false), isClosed(true), sendBuffer(nullptr)
...
boost::system::error_code Session::ReceiveWith(const string& localID, const string& remoteID, shared_ptr<string> buf) {
    if (IsClosed()) {
...
boost::system::error_code Session::handleACKPkt(shared_ptr<pb::Packet> pkt) {
...
boost::system::error_code Session::handleSeqPkt(shared_ptr<pb::Packet> pkt) {
...
boost::system::error_code Session::handleHandshakePacket(shared_ptr<pb::Packet> pkt) {
    if (isEstablished) {
        return ErrCode::Success;
    }
...
void Session::start() {
    std::thread* thrdFlush = new std::thread(std::bind(&Session::startFlush, this, config->FlushInterval));
    thrdFlush->detach();

    std::thread* thrdCheckRead = new std::thread(std::bind(&Session::startCheckBytesRead, this));
...
size_t Session::Read(byteSlice& buf) {
    ...
    if (!IsStream() && wanted < recvWindowData->at(recv_seq)->size()) { // Not support partial read in non-stream mode
