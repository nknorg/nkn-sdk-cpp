...
template<typename UnsupportedType, typename _Traits = UnsupportedType> struct Random {
    UnsupportedType operator()(...) {
        static_assert(!is_same<UnsupportedType,_Traits>::value,
...
// partial specialization for all container<non-primitive> instantiation
template<template<typename...> class _Tp, typename... Args>
struct Random< _Tp<Args...>,
               typename enable_if<
                    !is_integral<typename _Tp<Args...>::value_type>::value,
                    _Tp<Args...>
               >::type>
{
    typedef typename _Tp<Args...>::value_type _Elem;

    _Tp<Args...> operator()(...) {
        static_assert(is_integral<_Elem>::value,
...
// partial specialization for all container<primitive integer> instantiation
template<template<typename...> class _Tp, typename... Args>
struct Random< _Tp<Args...>,
               typename enable_if<
                    is_integral<typename _Tp<Args...>::value_type>::value,
